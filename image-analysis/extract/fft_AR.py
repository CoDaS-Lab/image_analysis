def filter(center_orientation, orientation_width, high_cutoff, low_cutoff,
    ...
    # theta = theta[0:target_size,0:target_size]; only need this to check dim's
    ...
    if falloff == 'rectangle':
        ...
        for idx, val in np.ndenumerate(theta):
            if ccwb1 <= val <= cwb1 and val <= center_orientation:
                anfilter[idx] = (val - center_orientation + width/2)*2/width
            elif ccwb1 <= val <= cwb1 and val > center_orientation:
                anfilter[idx] = (-val + center_orientation_2 + width/2)*2/width
        ...
        angfilter1 = np.exp(-((theta-center_orientation)/(.5*width))**4)
        angfilter1 = np.exp(-((theta-center_orientation_2)/(.5*width))**4)

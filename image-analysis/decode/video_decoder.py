...
def flatten(batch_list):
    ...
    OUTPUT
    returns a single list of frames that were in the original batch

    DESCRIPTION: takes a list of batches and unravels every frame inside the
    batches into a single list of frames. Reason for this is in case we just
    want all frames without being in batches.
    """
...
def pad_batch(batch, batch_size, frame):
    ...
    if len(batch) > batch_size:
        ...
        frame_ds = {
            'input': {
                'frame': np.zeros(frame['input']['frame'].shape)
            },
            'metadata': {

            }
        }
        pad_list = [frame_ds] * (batch_size - len(batch))
        batch += pad_list
        return batch
    else:
...
def decode_mpeg(v_path, *, batch_size=1, stride=1, start_idx=0, end_idx=-1):
    ...
    for frame in skvideo.io.vreader(v_path):
        ...
        if count == end_idx:
            ...
            if count >= idx and count < (idx + batch_size):
                batch.append(frame_ds)
                batch_list.append(np.array(
                    pad_batch(batch, batch_size, frame_ds)))
    ...
    """ this is for when we have the default value of batch_size=1
    instead of returning batches of 1 frame each just flatten the list
    of batches into a single list containing all the frames """
    if (batch_size == 1):
